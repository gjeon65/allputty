...
void do_bgfg(char **argv)
{
	...
	struct job_t * job2 = ((pid) ? getjobpid(jobs, id): getjobjid(jobs,id));
	if (job2 == NULL) {
		if(pid) {
			printf("(%d) no pid: ",id);
		} else {
	...
	while((pid = waitpid(-1, &status, WNOHANG|WUNTRACED)) > 0) {
		if(pid > 0) {
			if(WIFEXITED(status)) {
				...
				printf("job[%d] end: ",pid2jid(pid));
			 	}
				deletejob(jobs, pid);
			 }
			else if(WIFSTOPPED(status)) {
				getjobpid(jobs, pid)->state = ST;
				printf("job[%d] stop: ", pid2jid(pid));
				}
			}
		}
	return;
